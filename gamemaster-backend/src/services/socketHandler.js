...
function socketHandler(io, wsAuth) {
    ...
    io.on('connection', (socket) => {
        ...
        const userData = socket.data?.userData || { userId: socket.id, userName: 'Guest', authenticated: false };
        ...
        socket.on('find-room', (roomId, callback) => {
            try {
                ...
                if (!room)
                    return callback && callback({ ok: false, error: 'Session introuvable' });
                callback && callback({ ok: true, room: room.toJSON() });
            }
            catch (error) {
            ...
            try {
                ...
                if (!room) {
                    ...
                    if (typeof callback === 'function')
                        return callback({ ok: false, error: errPayload.message });
        ...
        socket.on('update-character', (data, callback) => {
            try {
                ...
                if (!isAllowed) {
                    if (typeof callback === 'function')
                        return callback({ ok: false, error: 'Accès refusé' });
        ...
        socket.on('chat-message', (data) => {
            try {
                ...
                const chatMessage = roomService.addChatMessage(roomId, userInfo.userId, userInfo.userName, message.trim());
        ...
        socket.on('set-current-scene', (data, callback) => {
            try {
                ...
                if (!userInfo || userInfo.userType !== 'gm') {
                    if (typeof callback === 'function')
                        return callback({ ok: false, error: 'Action réservée au GM' });
                ...
                if (!room) {
                    if (typeof callback === 'function')
                        return callback({ ok: false, error: 'Room non trouvée' });
                ...
                const scenarioObj = typeof room?.scenario === 'object' && room?.scenario !== null ? room.scenario : null;
                ...
                if (scenes) {
                    if (typeof sceneIndex === 'number' && scenes[sceneIndex]) {
                        ...
                        if (idx >= 0)
                            currentScene = { index: idx, ...scenes[idx] };
                    }
                }
                ...
                if (typeof callback === 'function')
                    return callback({ ok: true, currentScene });
            }
            catch (error) {
