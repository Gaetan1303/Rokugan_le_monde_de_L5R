...
router.post('/', (req, res) => {
    try {
        ...
        const gmId = uuidv();
        const room = roomService.createRoom(name.trim(), gmId, gmName.trim(), scenario?.trim() || 'Scénario libre', isPrivate, password?.trim());
        res.status(201).json({
            success: true,
...
router.post('/:roomId/join', (req, res) => {
    try {
        ...
        const player = roomService.addPlayerToRoom(roomId, playerId, playerName.trim(), character, password?.trim());
        ...
        if (error instanceof Error) {
            ...
            if (message.includes('mot de passe'))
                statusCode = 403;
            else if (message.includes('pleine'))
                statusCode = 409;
        }
...
router.post('/with-scenario', (req, res) => {
    try {
        ...
        const room = roomService.createRoom(name.trim(), gmId, gmName.trim(), scenarioData || 'Scénario libre', isPrivate, password?.trim());
        res.status(201).json({
            success: true,
            room: room.toJSON(),
            gmId,
...
router.put('/:roomId/scenario/scene', (req, res) => {
    try {
        ...
        const room = roomService.getRoomById(req.params.roomId);
        if (!room)
            return res.status(404).json({ success: false, message: 'Room non trouvée' });
        ...
        if (Array.isArray(scenariosScenes)) {
            if (typeof sceneIndex === 'number' && scenariosScenes[sceneIndex]) {
                ...
                if (idx >= 0)
                    currentScene = { index: idx, ...scenariosScenes[idx] };
            }
        }
